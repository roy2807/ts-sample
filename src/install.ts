...
function helpMenu() {
  ...
  console.log(
    `- ${colors.yellow("Restart")} - Restart the setup from the beginning.`,
  );
  console.log(
    `- ${colors.yellow("Skip")} - Skip the current step (use with caution).`,
  );
...
function FinalSynchronization(
  ...
  if (FinalCheck.length == 9) {
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 1 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
        ...
        answer.name == true ? WelcomeScreen() : console.log(
          `⚠️  ${
            colors.red.italic(
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 2 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 3 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 4 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 5 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 6 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 7 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
    ...
    console.log(
      `⚠️  ${
        colors.red.italic("Failed to install KadMap. cause: missed 8 steps")
      }`,
    );
    setTimeout(() => {
      const sync = async () => {
...
function EnableSevrice(
  ...
  setTimeout(() => {
    console.log(
      `✅${
        colors.bold("Services are now set to start automatically on boot.")
      }`,
    );
    FinalCheck.push("1");
...
function DeployApplications(
  ...
  console.log(
    `${
      colors.red("1")
    }. Core Applications (Video Call, Document Management, File Sharing)`,
  );
  ...
  const Deploy = async () => {
    ...
    const interval = setInterval(() => {
      const frame = frames[i++ % frames.length];
      logUpdate(
        `Setting up selected applications and creating workspaces${frame}`,
      );
    }, 150);
    ...
    setTimeout(() => {
      ...
      if (ans == "help") {
        ...
        console.log(`Type ${colors.green("help")} to see this menu help`);
        console.log(
          `- ${colors.yellow("Back")} - Return to the previous step.`,
        );
        console.log(
          `- ${
            colors.yellow("Restart")
          } - Restart the setup from the beginning.`,
        );
        console.log(
          `- ${
            colors.yellow("Skip")
          } - Skip the current step (use with caution).`,
        );
        console.log(`- ${colors.yellow("exit")} - Exit the installer.`);
        console.log(``);
        ...
        console.log(
          `${colors.bold("✅ Applications and workspaces are ready.")}`,
        );
        setTimeout(() => {
          FinalCheck.push("1");
        ...
        console.log(
          `${colors.yellow.italic("SyntaxError: Choose for Number 1,2,3 ")}`,
        );
        setTimeout(() => {
          Deploy();
        }, 1500);
...
function KadmapCoreService(
  ...
  setTimeout(() => {
    ...
    const interval = setInterval(() => {
      const frame = frames[i++ % frames.length];
      logUpdate(
        `Starting KadMap services (Directory Service, Exchange Service, Security Service, and others)${frame}`,
      );
    }, 150);
    ...
    setTimeout(() => {
      clearInterval(interval);
      console.log(
        `✅${colors.bold("KadMap core services are up and running.")}`,
      );
      FinalCheck.push("1");
...
function License(
  ...
  const license = async () => {
    const answer = await inquirer.prompt([{
      ...
      type: "input",
      message:
        "Enter the license key provided by your organization to activate KadMap on this machine:",
    }]);
    ...
    const interval = setInterval(() => {
      const frame = frames[i++ % frames.length];
      logUpdate(
        `Configuring network settings and applying license key${frame}`,
      );
    }, 150);
      ...
      if (ans == "help") {
        ...
        if (answer.name == ID) {
          ...
          console.log(
            `✅${
              colors.bold(
                " Network settings configured. Connected to MKDM, and license key applied successfully.",
              )
            }`,
          );
          console.log(``);
          ...
          setTimeout(() => {
            console.log(
              `⚠️  ${colors.red.italic("Could not connect to [")} ${
                colors.red.bold.italic(answer.name)
              } ${
                colors.red.italic(
                  "] : or verify the license key. Please check the information and try again.",
                )
              } .`,
            );
            license();
...
function ConfigSettings(
  ...
  const Configkadmap = async () => {
    const answer = await inquirer.prompt([{
      ...
      type: "input",
      message:
        "Enter the IP address or hostname of the Master KadMap Data Machine (MKDM):",
    }]);
        ...
        setTimeout(() => {
          console.log(
            `⚠️  ${colors.red.italic("Could not connect to [")} ${
              ...
              colors.red.italic(
                "]. please check your SSH credentials or Invald IpAdress and try again",
              )
            } .`,
          );
          Configkadmap();
...
function DownloadKadMap(
  ...
  const DownloadInternet = async () => {
    ...
    if (ans == "help") {
      ...
      console.log(
        `${colors.yellow.italic("SyntaxError: Choose for Number 1 or 2 ")}`,
      );
      setTimeout(() => {
        DownloadKadMap(AvailableRequirements, IPAddress, FinalCheck);
      }, 1500);
...
function PrepareEnvironment(
  ...
  const interval = setInterval(() => {
    const frame = frames[i++ % frames.length];
    logUpdate(
      `Preparing environment: updating packages, installing Docker, Docker Compose, and network utilities${frame}`,
    );
  }, 150);
  ...
  setTimeout(() => {
    ...
    console.log(
      `${
        colors.bold(
          "✅ Environment setup complete. The machine is ready for KadMap components. ",
        )
      }`,
    );
    DownloadKadMap(AvailableRequirements, IPAddress, FinalCheck);
...
// goood
// Selling point +1
function CheckSystemRequierment(
  AvailableRequirements: string[],
  IPAddress: string,
  FinalCheck: string[],
) {
  ...
  if (
    AvailableRequirements.includes("3.8GHz") &&
    AvailableRequirements.includes("128GB") &&
    AvailableRequirements.includes("1TG SSD")
  ) {
    console.log(
      ` ✅${
        colors.bold(
    ...
    console.log(
      `[List specific missing requirements, e.g., insufficient memory, Docker not installed]`,
    );
  }
}
      ...
      const interval = setInterval(() => {
        ...
        logUpdate(
          `Attempting to connect to [${
            colors.bold.italic(answer2.name)
          }]${frame}`,
        );
      }, 150);
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          if (IPAddress == answer2.name) {
            ...
            console.log(
              `⚠️  ${colors.red.italic("Could not connect to [")} ${
                colors.red.bold.italic(answer2.name)
              } ${
                colors.red.italic(
                  "]. Please check the network connection or SSH credentials and try again",
                )
              } .`,
            );
            TargetMachine();
...
const WelcomeScreen = async () => {
  ...
  console.log(
    `${colors.red("2")}. You have SSH credentials ${
      colors.red("for")
    } the target machine.`,
  );
  console.log(
    `${colors.red("3")}. The organization admin is available ${
      colors.red("for")
    } license and MKDM details.`,
  );
  console.log(
    `Type ${colors.green("help")} at any step ${
      colors.red("for")
